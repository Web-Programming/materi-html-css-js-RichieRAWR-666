...
function renderProducts() {
  ...
  products.forEach(product => {
    ...
    if (product.diskon > 0) {
      ...
      priceHTML = `<span class="text-muted text-decoration-line-through small">
      Rp ${product.diskon.toLocaleString()}</span><br>
    ...
    if (product.stok > 0) {
      buttonHTML = `<button class="btn btn-buy p-2"
      onclick="alert('Ditambahkan ke keranjang: ${product.nama}')">
...
function renderProducts() {
  ...
  // 4. DOM MANIPULATION: Membuat template HTML
  // untuk setiap card
  const productHTML =
  `<div class="col-12 col-md-6 col-lg-4 col-xl-3">
      ...
      <div class="card-body">
        ...
        text-muted" style="font-size: 0.7rem;">
...
function renderProducts() {
  products.forEach(product => {
    ...
    const productHTML = `
      <small class="text-uppercase text-muted" style="font-size: 0.7rem;">
        ${product.kategori}
      </small>

      <h6 class="card-title fw-bold mb-3 mt-1">
        ${product.nama}
      </h6>
